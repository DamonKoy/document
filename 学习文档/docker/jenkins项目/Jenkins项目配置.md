### Jenkins 项目[chuman-qa-platform-server ](http://192.168.216.210:8080/job/chuman-qa-platform-server/)

``` shell
#!/bin/bash
# 执行项目脚本

# ps -ef | grep gunicorn | grep -v grep | awk '{print $2}'| xargs sudo kill -9
...
source set_flask_env.sh prod 

export PATH=$PATH:/var/jenkins_home/python3/bin/

gunicorn manage:app -c gunicorn.conf.py

sleep 3

pid_result=`ps -ef | grep gunicorn | grep -v grep | awk '{print $2}'`
...
if [ $test_env == 'none' ];then
  ...
  if [ -f $new_report_html ];then
    ...
    new_show_report_name="qa-p1p2报告_allure_report_$BUILD_NUMBER"
    ...
    broken_module=''
      if [ $broken_num != '0' ] || [ $failed_num != '0' ];then
        for index in `seq 0 $run_num`
        do
          result=$(cat $status_json | jq ".[$index].status")
          echo $result
          if [[ $result == '"broken"' ]] || [[ $result == '"failed"' ]];then
...
rm -rf ${WORKSPACE}/allure_report/

#!/bin/bash
lsof -i:22471|sed -n '2p'|awk '{print $2}' | xargs kill -9 || True
ps -ef |grep Nemu |grep -v grep |awk '{print $2}' |xargs kill -9 || True
...
result=""
while [ "$result" == "" ]; do
    echo "appium still not started!Try again!"
    bash startAppium.sh
    sleep 2
    appium_log_content=`cat appium.log`
    result=$(echo ${appium_log_content} | grep "${test}")
done 
ps -ef |grep appium
cat appium.log
sleep 2
